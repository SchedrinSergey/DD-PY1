...
# Реализуем функцию to_csv_file, которая записывает данные в файл output.csv
...
def to_csv_file(filename, headers, rows, delimiter=',', new_line='\n'):
    with open(filename, 'w') as f:
        f.write(delimiter.join(headers)+new_line)  # заносим в файл заголовки
        for row in rows:
            f.write(delimiter.join(row)+new_line)  # заносим в файл данные
